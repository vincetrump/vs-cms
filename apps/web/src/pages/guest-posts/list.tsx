...
export const GuestPostList = () => {
  const { tableProps, tableQuery } = useTable({ resource: "guest-posts", syncWithLocation: true });
  ...
  const statusHints: Record<string, string> = {
    active: "Bài viết đang được deploy trên các websites",
  ...
  const handleToggle = async (id: string) => {
    try {
      await axiosInstance.post(`${API_URL}/guest-posts/${id}/toggle`);
  ...
  const renderMobileActions = (record: any) => {
    const items: any[] = [
      { key: "show", label: "View", onClick: () => show("guest-posts", record._id) },
      { key: "edit", label: "Edit", onClick: () => edit("guest-posts", record._id) },
    ];
    if (isAdmin) {
      items.push({
        key: "toggle",
  ...
  const renderDesktopActions = (record: any) => (
    <Space size={4}>
      ...
      {isAdmin && (
        <Popconfirm
          title={record.status === "active" ? "Disable bài viết này?" : record.status === "pending" ? "Approve bài viết này?" : "Enable bài viết này?"}
          description={
            record.status === "active" ? "Gỡ bài viết khỏi tất cả websites" :
            record.status === "pending" ? "Deploy bài viết lên các websites đã chọn" :
  ...
  return (
    <List
      headerButtons={
        <Button type="primary" icon={<PlusOutlined />} onClick={() => create("guest-posts")}>
          {screens.sm ? "New Guest Post" : "New"}
        </Button>
      }
    >
      <Table
      ...
      >
        ...
        {screens.sm && (
          <Table.Column
            dataIndex="realPublic"
            title="SEO"
            width={90}
            render={(v: boolean) => (
              <Tooltip title={v ? "Cho phép bot index, đã đưa vào sitemap" : "Chặn bot index (noindex), không đưa vào sitemap"}>
                <Tag color={v ? "green" : "default"}>{v ? "Public" : "NoIndex"}</Tag>
              </Tooltip>
            )}
            filters={[
              { text: "Public", value: true },
              { text: "NoIndex", value: false },
            ]}
          />
        )}
        {screens.sm && (
